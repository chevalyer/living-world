...
public sealed class TeachAction : SimAction
{
    ...
    public override IEnumerable<ActionOption> Options(PlanningContext c)
    {
        ...
        foreach (var person in c.OfKind("npc").Where(o=>o.Age>=3)) foreach (var skill in c.Skills.Experience.Where(x=>x.Value>person.Skills.GetValueOrDefault(x.Key)+15).Take(2))
        {
    ...
    public override bool Execute(SimulationSession s, int actor, ActionStep step)
    {
        ...
        foreach (var fact in knowledge.Facts.Take(1))learned.Facts.Add(fact);
        foreach (var plant in knowledge.EdiblePlants)learned.EdiblePlants.Add(plant);
        foreach (var fact in knowledge.Facts.Where(f=>!learned.Facts.Contains(f)).Take(1))learned.Facts.Add(fact);
