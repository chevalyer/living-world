        ...
        for (var radius=1;radius<=8;radius++)
            for (var dy=-radius;dy<=radius;dy++)
                for (var dx=-radius;dx<=radius;dx++)
                {
                    ...
                    if (!CanPlace(session,origin))continue;
                    GridPoint[] visibleChecks=
                    [
                        origin,
                        origin+new GridPoint(Width-1,0),
                        origin+new GridPoint(0,Height-1),
                        origin+new GridPoint(Width-1,Height-1)
                    ];
                    if (visibleChecks.All(p=>p.Distance(observer)<=12&&PerceptionSystem.LineOfSight(s.Map,observer,p)))
                        return origin;
                }
        return null;
    }
