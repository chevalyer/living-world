...
public sealed class PhysiologyEvaluator : ISituationEvaluator
{
    ...
    {
        var calories=Math.Max(1,(int)MathF.Ceiling(Math.Max(0,c.Needs.Hunger-.25f)*2400));
        yield return new("fed","голод",MathF.Pow(c.Needs.Hunger,3)*12,calories);
        ...
        if(c.CanWork&&((!c.Sheltered&&c.Family.HomeProject==0)||c.Known.Any(o=>o.Kind=="project"&&o.Entity==c.Family.HomeProject&&o.Quantity>0)))
            yield return new("housing_progress","защита от холода и непогоды",.5f+c.Personality.Caution*.8f+Math.Max(0,15-c.Air)*.07f);
    }
}
