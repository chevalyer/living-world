...
public sealed class NameGenerator(NameDefinition names)
{
    ...
    {
        return (Word(random, names.FirstMinSyllables, names.FirstMaxSyllables),
            Word(random, names.LastMinSyllables, names.LastMaxSyllables));
    }

    private string Word(DeterministicRandom random, int minimum, int maximum)
    {
        ...
        for (var syllable = 0; syllable < count; syllable++)
        {
            ...
            foreach (var requested in pattern)
            {
                ...
                // No doubled letters or runs of three vowels/consonants, including syllable boundaries.
