...
public static class TestSuite
{
    ...
    public static int Run()
    {
        ...
        {
            var(s, id)=Fixture(); var project=BuildHome(s, id); new RoomSystem().Update(s); var home=s.State.Entities.Get<ConstructionComponent>(project); Assert(s.State.Rooms.Count>0, "no room"); Assert(home.Finished, "unfinished"); Assert(s.State.Map.Walkable(home.Door), "door blocks movement");
        });
        Test("opening wall invalidates room", ()=>
        {
            var(s, id)=Fixture(); var project=BuildHome(s, id); new RoomSystem().Update(s); var wall=s.State.Entities.Get<ConstructionComponent>(project).Elements.First(x=>x.Kind=="wall").Position; s.State.Map[wall].Wall=0; s.RoomsDirty=true; new RoomSystem().Update(s); Equal(0, s.State.Rooms.Count);
        });
        ...
        Test("facilities use the exact procedural home interior", ()=>
        {
            var(s,id)=Fixture();
            var project=BuildHome(s,id);
            new RoomSystem().Update(s);
            var home=s.State.Entities.Get<ConstructionComponent>(project);
            var site=FacilityService.FindSite(s,id,D.Facilities["bed"]);
            Assert(site.HasValue,"no indoor facility site in completed procedural home");
            Assert(home.Interior.Contains(site.Value),"facility site escaped the generated home interior");
        });
