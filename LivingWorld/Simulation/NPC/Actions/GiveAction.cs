...
public sealed class GiveAction : SimAction
{
    ...
    public override IEnumerable<ActionOption> Options(PlanningContext c)
    {
        ...
        foreach (var person in c.OfKind("npc").Where(o=>o.Need>.6f))
        foreach (var d in c.Definitions.Items.Values.Where(d=>d.Calories>0))
        {
            ...
            op.Effects=[new(Item(d.Id),-1),new($"helped:{person.Entity}",1,true)];
            yield return op;
        }
    }
    public override bool Execute(SimulationSession s,int actor,ActionStep step)
    {
        ...
        if (id==0||s.State.Entities.Get<ItemComponent>(id).Freshness<.1f||s.State.Entities.Get<NeedsComponent>(actor).Hunger>.5f)return false;
