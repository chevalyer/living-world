...
public sealed class TalkAction : SimAction
{
    ...
    public override bool CanExecute(SimulationSession s, int actor, ActionStep step, out string reason)
    {
        ...
        return s.State.Entities.Try<HealthComponent>(step.Target) is
        {
            Alive:true
        }
    ...
    public override bool Execute(SimulationSession s, int actor, ActionStep step)
    {
        ...
        var shared=other.Observations.Where(o=>o.Kind is "water" or "plant" or "project").OrderByDescending(o=>o.SeenTick).Take(5);
        foreach (var memory in shared)if (!receiver.Observations.Any(o=>o.Kind==memory.Kind&&o.Position==memory.Position))receiver.Observations.Add(memory with
        {
            Confidence=memory.Confidence*.7f, Skills=new(memory.Skills), Items=new(memory.Items)
        });
