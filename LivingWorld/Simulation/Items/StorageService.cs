...
public static class StorageService
{
    public static int Create(SimulationSession session, GridPoint p, int project)
    {
        ...
        e.Set(id, new StorageComponent
        {
            Project=project
        });
        e.Set(id, new InventoryComponent
        {
            MaxMass=500, MaxVolume=800
        });
    ...
    public static bool Take(SimulationSession session, int actor, int storage, string definition)
    {
        var e=session.State.Entities;
        if (!e.Has<StorageComponent>(storage))return false;
        var item=session.Inventory.Items(storage).FirstOrDefault(id=>e.Get<ItemComponent>(id).Definition==definition&&(e.Get<OwnershipComponent>(id).Owner==0||e.Get<OwnershipComponent>(id).Owner==actor));
